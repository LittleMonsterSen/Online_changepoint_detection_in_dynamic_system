...
def elbo_decomposition(vae, dataset_loader):
    ...
    for xs in dataset_loader:
        ...
        #xs = Variable(xs.view(batch_size, -1, 32, 32).cuda(), volatile=True)
        xs = Variable(xs.view(batch_size, -1, 32, 32), volatile=True)
        zs, z_params = vae.encode(xs)
        z_params = z_params.view(batch_size, K, nparams)
